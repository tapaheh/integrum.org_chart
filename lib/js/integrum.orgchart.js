(function ( $ ) {
    ...
    $.fn.orgchart = function( options ) {
        ...
        var siblings = {'p1': [], 'd1': [], 'p2': [], 'd2': [], 'root': []} //элементы в группе
                ...
                if (sName == 'p1' || sName == 'd1') {
                    ...
                    _.each(siblings[sName], function (elem, i) {
                        ...
                        if (relatives.length > 0) {
                            var min_relative = false;
                            var max_relative = false;
                            var min_relative_index = false;
                            var max_relative_index = false;
                            _.each(relatives, function (elemId, j) {
                                if (j == 0) {
                                    min_relative_index = paper.findViewByModel(elemId).elemIndex;
                                    max_relative_index = paper.findViewByModel(elemId).elemIndex;
                                    min_relative = elemId;
                                    max_relative = elemId;
                                }
                                if (min_relative_index > paper.findViewByModel(elemId).elemIndex) {
                                    min_relative_index =  paper.findViewByModel(elemId).elemIndex;
                                    min_relative = elemId;
                                }
                                if (max_relative_index < paper.findViewByModel(elemId).elemIndex) {
                                    max_relative_index =  paper.findViewByModel(elemId).elemIndex;
                                    max_relative = elemId;
                                }
                            });
                            ...
                            edges[sName].push({parentId:elem.model.id, 'id': 'link_' + elem.model.id, 'y': prev_rank_y, 'curr_y':prev_rank_y, 'min_x': false, 'max_x': false, 'source': elem.model.id, 'relatives': relatives, 'min_relative':min_relative, 'max_relative':max_relative});
                        }
                    }, this);
                    updateEdges(sName);
                }
        ...
        var updateEdges = function(group) {
            ...
            _.each(edges[viewGroup], function (edge) {
                ...
                edge.max_x = paper.findViewByModel(edge.min_relative).x_center;
                edge.min_x = paper.findViewByModel(edge.max_relative).x_center;
                ...
                for (var i = 0; i < checkedEdges.length; i++) {
        ...
        joint.shapes.basic.OrgView = joint.dia.ElementView.extend({
            ...
            updatePosition: function (is_setup, dx) {
                ...
                if (new_is_visible || is_setup) {
                    ...
                    if (distanceFactor < 0.2){
                        this.$el.find('.node text').hide();
                    } else {
                        this.$el.find('.node text').show();
                    }
                ...
                if (dx != 0)
                {
                    ...
                    if (dx > 0) {
                        for (var i = 0; i < siblings[this.group].length; i++) {
                            var view = paper.findViewByModel(siblings[this.group][i].model.id).updatePosition(false, dx);
                        }
                    } else if (dx < 0) {
